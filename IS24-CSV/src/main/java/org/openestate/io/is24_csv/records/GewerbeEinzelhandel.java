...
public class GewerbeEinzelhandel extends Is24CsvRecord
{
  ...
  public Double getDeckenlast()
  {
    try
    {
      return this.getAsDouble( FIELD_DECKENLAST );
    }
    catch (NumberFormatException ex)
    {
      LOGGER.warn( "Can't read 'Deckenlast' "
        + "from '" + this.get( FIELD_DECKENLAST ) + "'!" );
      return null;
    }
  }
  ...
  public Lage getLage()
  {
    return Lage.parse( this.get( FIELD_LAGE ) );
  }
  ...
  {
    return ObjektkategorieGewerbeEinzelhandel.parse( this.get( FIELD_OBJEKTKATEGORIE ) );
  }
  ...
  public Double getSchaufensterfront()
  {
    try
    {
      return this.getAsDouble( FIELD_SCHAUFENSTERFRONT );
    }
    catch (NumberFormatException ex)
    {
      LOGGER.warn( "Can't read 'Schaufensterfront' "
        + "from '" + this.get( FIELD_SCHAUFENSTERFRONT ) + "'!" );
      return null;
    }
  }
  ...
  {
    return Zulieferung.parse( this.get( FIELD_ZULIEFERUNG ) );
  }
